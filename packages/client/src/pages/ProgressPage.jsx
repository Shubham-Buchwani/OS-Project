        ...
        <div className="glass-card" style={{ padding: 22, marginBottom: 32 }}>
          ...
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
            {progress.achievements.map((a) => (
              <div key={a.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 14px', background: 'var(--clr-warning-dim)', border: '1px solid rgba(245,158,11,0.3)', borderRadius: 8 }}>
                <Trophy size={16} style={{ color: 'var(--clr-warning)' }} />
                <div>
                  <div style={{ fontSize: '0.8rem', fontWeight: 600, color: 'var(--clr-warning)' }}>{a.title}</div>
                  <div style={{ fontSize: '0.7rem', color: 'var(--clr-text-muted)' }}>{new Date(a.unlockedAt).toLocaleDateString()}</div>
                </div>
              </div>
            ))}
          </div>
